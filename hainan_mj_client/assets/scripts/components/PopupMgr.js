cc.Class({
    ...
    onLoad: function () {
        ...
        this._jiesanTip.active = false;
        this.closeAll();
        
        this.addBtnHandler("settings/btn_close");
        this.addBtnHandler("settings/btn_sqjsfj");
        this.addBtnHandler("settings/btn_game_exit");
        this.addBtnHandler("dissolve_notice/btn_agree");
        this.addBtnHandler("dissolve_notice/btn_reject");
        
        var btn_sqjsfj = cc.find("Canvas/popups/settings/btn_sqjsfj");
        if (btn_sqjsfj) {
            btn_sqjsfj.active = cc.vv.replayMgr.isReplay() == false;
        }
    ...
    start:function(){
        if(cc.vv.MjNetMgr.dissoveData){
            this.showDissolveNotice(cc.vv.MjNetMgr.dissoveData);
        }
    },
    ...
    onBtnClicked:function(event){
        ...
        else if(btnName == "btn_game_exit"){
            if(cc.vv.MjNetMgr.isOwner()){
    ...
    showSettings:function(){
        ...
        var exit = this._settings.getChildByName("btn_game_exit");
        dis.active = !cc.vv.replayMgr.isReplay() && cc.vv.MjNetMgr.numOfGames > 0;
        exit.active = !cc.vv.replayMgr.isReplay() && cc.vv.MjNetMgr.numOfGames == 0;
    },
    ...
    showDissolveNotice:function(data){
        ...
        for(var i = 0; i < data.states.length; ++i){
            ...
            var name = cc.vv.MjNetMgr.seats[i].name;
            if(b == 2){
                this._extraInfo += "\n "+ name + "   [已同意]";
                if(localIndex == 0 ){
        ...
        this._jiesanTip.active = cc.vv.MjNetMgr.numOfGames == 1;
    },
