...
exports.get_act_content = function(callback){
    callback = callback == null ? nop : callback;
    var sql = 'SELECT * FROM t_act WHERE enable = 1';
    query(sql, function (err, rows, fields) {
        if (err) {
            callback(null);
        }
...
exports.get_user_data = function (account, callback) {
    ...
    query(sql, function (err, rows, fields) {
        ...
        // console.log("获取玩家的名字"+rows[0].name);
        if (rows[0].name) {
...
exports.update_user_coins = function (user, admin, coins, callback) {
    callback = callback == null ? nop : callback;
    if (!user || !admin) {
        callback(false);
        return;
    }
    var sql1 = 'UPDATE t_users SET coins = coins + ' + coins + ' WHERE userid = ' + user + " ; "
    var sql2 = 'UPDATE t_users SET coins = coins - ' + coins + ' WHERE userid = ' + admin + " ; "
    var sql = sql1 + sql2;
...
exports.update_user_score = function (roomId, scoreList, table, callback) {
    ...
    var sql = 'UPDATE ' + table + ' SET user_score0 = ' + scoreList[0] + ',user_score1 = ' + scoreList[1] + ',user_score2 = ' + scoreList[2] + ',user_score3 = ' + scoreList[3] + ' where id = ' + roomId;
...
//更改玩家的金币数
exports.change_user_actNum = function (userId, coins, callback) {
    callback = callback == null ? nop : callback;
    if (userId == null) {
        callback(null);
        return;
    }
    var sql = 'UPDATE t_users SET coins= coins + {0} WHERE userId={1}';
    sql = sql.format(coins, userId);
    console.log("change_user_actNum == " + sql);
    query(sql, function (err, rows, fields) {
        if (err) {
           console.log(err)
        }
...
exports.update_user_cer = function (userid, real_name, carId, cer, callback) {
    callback = callback == null ? nop : callback;
    if (userid == null) {
        callback(null);
        return;
    }
    var sql = 'UPDATE t_users SET real_name="{0}",id_card_img={1},certification={2} WHERE userid={3}';
    sql = sql.format(real_name, carId, cer, userid);
    console.log("update_user_cer", sql);
    query(sql, function (err, rows, fields) {
        if (err) {
            console.log(err);
            callback(null)
...
exports.select_quan_room = function (quanId, callback) {
    callback = callback == null ? nop : callback;
    var sql = 'SELECT * FROM t_rooms WHERE quanid = "' + quanId + '"'; //t_rooms
    console.log("select_quan_room = " + sql);
    query(sql, function (err, rows, fields) {
        if (err) {
            callback(false);
            console.log("select_quan_room err" + err)
        } else {
            callback(rows.length > 0);
        }
    });
...
exports.set_userid_of_room = function (userid, roomId, userName, callback) {
    callback = callback == null ? nop : callback;
    var sql = 'UPDATE t_rooms SET ' + userid + '= 0 , ' + userName + '= \'\' , isKong = 0  WHERE id=' + roomId;
...
exports.update_room_iskong = function (roomId, iskong, callback) {
    callback = callback == null ? nop : callback;
    var sql = 'UPDATE t_rooms SET isKong = '+ iskong +' WHERE id = '+roomId;
    console.log("update_seat_info == " + sql);
    query(sql, function (err, row, fields) {
        if (err) {
            console.log("update_seat_info err ==  " + err)
            callback(false);
        } else {
            callback(true);
        }
...
exports.update_next_button = function (roomId, nextButton, gameLing, callback) {
    callback = callback == null ? nop : callback;
    var sql = 'UPDATE t_rooms SET next_button = {0} , game_ling = {2} WHERE id = "{1}"';
    sql = sql.format(nextButton, roomId, gameLing);
    console.log(sql);
    query(sql, function (err, row, fields) {
        if (err) {
            callback(false);
...
exports.create_zhanji_info = function (roomid, quanid, quanzhu, callback) {
    callback = callback == null ? nop : callback;
    var sql = "INSERT INTO t_zhanji_info(roomid,quanid,quanzhu,createtime) VALUES('{0}',{1},'{2}',unix_timestamp(now()))";
    sql = sql.format(roomid, quanid, quanzhu);
    console.log(" 创建战绩信息 sql === " + sql);
    query(sql, function (err, rows, fields) {
        if (err) {
            callback(null);
        } else {
            console.log("创建战绩返回的信息 === " + rows.insertId)
            callback(rows.insertId);
        }
    });
};
exports.store_sigle_zhanji = function (userid, username, score, index, id, callback) {
    callback = callback == null ? nop : callback;
    var ud = "userid" + index;
    var un = "username" + index;
    var us = "userscore" + index;
    var sql = "UPDATE t_zhanji_info SET " + ud + "=" + userid + "," + un + "='" + username + "'," + us + "=" + score + " WHERE id = " + id;
    console.log("保存战绩 " + sql);
    query(sql, function (err, rows, fields) {
        if (err) {
            callback(false);
            throw err;
...
exports.store_quan_rule = function (quanId,key,rule, callback) {
    callback = callback == null ? nop : callback;
    var sqlu = 'UPDATE t_quan SET '+ key +' = \''+ rule +'\' where id = ' + quanId;
    
    console.log("store_quan_rule sql ==" + sqlu)
    query(sqlu, function (err, rows, fields) {
        if (err) {
            callback(false)
            console.log("store_quan_rule" + err)
        } else {
            callback(true)
        }
    })
...
exports.update_user_quan = function (quan, userId, callback) {
    ...
    query(sql, function (err, rows, fields) {
        if (err) {
            ...
            console.log("把玩家添加的亲友圈里成功22")
            callback(true)
        }
    })
};
exports.get_quan_zhuos = function (quanId, params, callback) {
    callback = callback == null ? nop : callback;
    let p = params ? params.toString() : "*"
    var sql = "select " + p + " from t_rooms where quanid = " + quanId;
    query(sql, function (err, rows, fields) {
        if (err) {
...
exports.select_user_arr = function (arr, canArr, callback) {
    callback = callback == null ? nop : callback;
    let canStr = canArr ? canArr.toString() : "*"
    var sql = "select " + canStr + " from t_users where userid in (" + arr.toString() + ") order by find_in_set(userid,'" + arr.toString() + "')";
    console.log("hhhh", sql)
    query(sql, function (err, rows, fields) {
        if (err) {
            console.log("select_user_arr err = " + err)
            callback(false);
        } else {
            callback(rows);
        }
...
exports.select_quan_arr = function (arr, canArr, callback) {
    callback = callback == null ? nop : callback;
    let canStr = canArr ? canArr.toString() : "*"
    var sql = "select " + canStr + " from t_quan where id in (" + arr.toString() + ") order by find_in_set(id,'" + arr.toString() + "')";
    query(sql, function (err, rows, fields) {
        if (err) {
            console.log("select_quan_arr err = " + err)
            callback(false);
        } else {
            callback(rows);
        }
...
exports.update_user_quan_message = function (quanId, userId, creator, quanName, uname, aaa, callback) {
    ...
    query(sql, function (err, rows, fields) {
        if (err) {
            ...
            console.log("亲友圈信息申请成功")
            callback(true)
        }
    })
};
exports.get_re_message = function (quanId, callback) {
    callback = callback == null ? nop : callback;
    var sql = "select * from t_quan_message where quanid = " + quanId + " AND status = 0  ";
...
exports.get_quan_applay_message = function (userid, callback) {
    callback = callback == null ? nop : callback;
    var sql = "select * from t_quan_message where applicant = " + userid;
    console.log("get_quan_applay_message == " + sql);
    query(sql, function (err, rows, fields) {
        if (err) {
            console.log("get_quan_applay_message err =" + err)
        } else {
            callback(rows);
        }
    })
};
exports.change_quan_message_status = function (userId, status, quanId, callback) {
    callback = callback == null ? nop : callback;
...
exports.store_history = function (uuid, history, quanid,users, callback) {
    callback = callback == null ? nop : callback;
    ...
    var sql = 'INSERT INTO t_history(uuid,history,quanid,storetime,users) VALUES(' + uuid + ', \'' + history + '\' , "' + quanid + '" ,' + store_time + ',"' + users + '" )';
...
exports.get_history = function (userId, callback) {
    callback = callback == null ? nop : callback;
    if (!userId) {
        callback([]);
        return;
    }
    var sql = 'SELECT * FROM t_history WHERE users LIKE "%' + userId + '%"';
...
exports.get_other_history = function (uuid, callback) {
    callback = callback == null ? nop : callback;
    if (!uuid) {
        callback(null);
        return;
    }
    var sql = 'SELECT history FROM t_history WHERE uuid =' + uuid;
    console.log("get_other_history sql == " + sql);
    query(sql, function (err, rows, fields) {
        if (err) {
            console.log("get_other_history err = " + err)
            callback(null);
        } else {
            callback(rows[0]);
        }
...
exports.get_quan_history_user = function (quanId,userId, callback) {
    callback = callback == null ? nop : callback;
    if (!quanId) {
        callback([]);
        return;
    }
    var sql = 'SELECT * FROM t_history WHERE quanid = "' + quanId + '" AND users LIKE "%'+userId+'%"';
    console.log("get_quan_history_user sql == " + sql);
    query(sql, function (err, rows, fields) {
        if (err) {
            console.log("get_quan_history_user err = " + err)
            callback([]);
        } else {
            console.log("获取个人圈历史记录数目 == " + rows.length)
            if (rows.length == 0) {
                callback([]);
            }
            else{
